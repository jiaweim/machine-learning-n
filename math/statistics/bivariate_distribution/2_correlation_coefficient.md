...
下面介绍如何量化两个随机变量 $X$ 和 $Y$ 的相关程度。假设 $X$ 表示一个咖啡店每天售出的热巧克力的杯数，$Y$ 表示该咖啡店每天售出的苹果肉桂松饼的个数。现在咖啡店的经理想知道这两者之间是否相关。如果它们高度相关，那么经理可能会考虑尽量同时提供这两个商品；如果不相关，则在提供一个商品时、不需要保证同时提供另一个商品。**相关系数**（correlation coefficient）是一个统计量，用于量化两个随机变量之间的相关程度。
...
下面，我们通过研究两个随机变量 $X$ 和 $Y$ 之间的协方差，开始尝试量化两个随机变量之间的相关性。首先给出协方差的定义。
...
对第一个问题，如何理解协方差？实际上，我们用协方差来计算相关系数，后面会详细解释。而对第二个问题，我们来看一个定理。
