...
下面介绍如何量化两个随机变量 $X$ 和 $Y$ 的相关程度。假设 $X$ 表示一个咖啡店每天售出的热巧克力杯数，$Y$ 表示该咖啡店每天售出的肉松饼个数。现在咖啡店的老板想知道这两者之间是否相关。如果它们高度相关，那么老板可能会尽量同时提供这两个商品；如果不相关，则在提供一个商品时、不需要保证同时提供另一个商品。**相关系数**（correlation coefficient）是一个统计量，用于量化两个随机变量之间的相关程度。
