...
**集中趋势指标**，指一个代表数据集的典型值或中心值的数值。三种最常用的指标为：

- 平均值（mean）：算术平均值
- 中位数（median）：对数据排序后中间的值
- 众数（mode）：最常出现的值

用分布特征来描述这些指标。假设 $X$ 是一个概率密度函数为 $f_X$、累计分布函数为 $F_X$ 的**连续随机变量**。那么 

- mode 定义为  $mode(X)=\arg \max_x f_X(x)$
- median 定义为 $\text{median}(X)=m$，其中 $m$ 满足 $F_X{m}=0.5$

下面来看**平均值**（mean） 定义：

随机变量 $X$ 的 mean，记为 $E(X)$，定义为：
$$
E(X)=
\begin{cases}
    \sum_x xf_X(x) & X 离散\\
    \int_{-\infty}^\infty xf_X(x)dx & X 连续
\end{cases} \tag{1}
$$

通常用希腊字母 $\mu$ 表示平均值。显然，平均值可以看作分布的一个参数。$X$ 的平均值又称为 $X$ 的**期望值**（expectation）。在讨论随机变量的属性时，还经常使用 $E(X)$ 来表示平均值。

**示例（二项分布均值）**

假设 $X \sim Bin(n,p)$，其概率函数为：
$$
f_X(x)=\binom{n}{x} p^x(1-p)^{n-x}, \quad x=0,...,n
$$

根据（1）式，$X$ 的平均值为：

$$
E(X)=\sum_x xf_X(x)=\sum_{x=0}^n x\binom{n}{x}p^x(1-p)^{n-x}
$$

余下就是级数求和的一些技巧。最终可以得到：
...
假设 $X\sim Exp(\lambda)$，那么 $X$ 的平均值定义为：
...
对随机变量 $X$ 和正整数 $r$，其 $r$ 阶矩 (moment) $\mu_r'$ 定义为：
$$
\mu_r'=E(X^r)
$$
moment 依赖于分布的水平位置。当衡量分布的特征时，我们有时候希望当分布向左或向右移动，特征值保持不变。于是有了**中心距**（central moment）的定义。

对随机变量 $X$ 和正整数 $r$，$X$ 的 r 阶中心矩 $\mu_r$ 定义为：
$$
\mu_r=E[(X-E(X))^r]
$$
...
> [!TIP]
>
> 一阶矩常用于描述数据的集中趋势，二阶中心矩则用于描述数据的分散程度。
