...
- [保存和加载模型](#保存和加载模型)
  ...
  - [设置](#设置)
    ...
    - [获取示例数据](#获取示例数据)
    - [定义模型](#定义模型)
  - [训练时保存 checkpoints](#训练时保存-checkpoints)
    - [Checkpoint callback 使用](#checkpoint-callback-使用)
    - [Checkpoint callback 选项](#checkpoint-callback-选项)
  - [文件说明](#文件说明)
  - [手动保存权重](#手动保存权重)
  - [保存整个模型](#保存整个模型)
...
在训练过程中和训练后都可以保存模型，这样模型可以从停止的地方恢复，以避免长时间的训练。能够保存模型也意味着可以共享模型，在发布模型和技术研究工作时，大多会共享：
...
根据所使用的 API，保存 TF 模型的方法也不同。本教程使用 tf.keras API 构建和训练模型。其它方法可以参考 [Using the SavedModel format](https://www.tensorflow.org/guide/saved_model) 和 [Save and load Keras models](https://www.tensorflow.org/guide/keras/save_and_serialize).
...
`tf.keras.callbacks.ModelCheckpoint` callback 用于在训练期间和训练后保存模型。

### Checkpoint callback 使用

创建一个在训练期间**保存权重**的 `tf.keras.callbacks.ModelCheckpoint` callback:
